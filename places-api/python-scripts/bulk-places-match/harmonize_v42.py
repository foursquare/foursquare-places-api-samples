...
csv.register_dialect('piper', delimiter='|', quoting=csv.QUOTE_NONE,escapechar='\\')
...
parser = argparse.ArgumentParser(description='Match / Harmonize places against Foursquare API')
parser.add_argument("--input",required=True, help="Input filename")
parser.add_argument("--output",required=False, help="Output filename, defaults to <input filename>.out")
parser.add_argument("--separator",required=False, default="T",help="Separator in the CSV file (C)omma,(T)ab,(P)ipe,(S)emi(C)olon",choices=["C","T","P","SC"])
parser.add_argument("--country",required=False,help="Country")
parser.add_argument("--match-type",required=False,default="ALL",help="Specify one of ALL, NAME, ADDRESS")
parser.add_argument("--column_mapping",required=False, default='{"id":"id_col","name":"name_col","address":"address_column","city":"city_col","state":"state_col","zip":"zip_col","cc":"country_col","lat":"lat_col","lng":"lon_col","ll":"ll_col"}',help="Field to column mapping for id, name, address, city, state, zip, country, lat, lng, ll")
...
ID_COL= MAPPING["id"] if "id" in MAPPING else print("Error: You need a correctly mapped 'id' column in the input file")
...
OUTPUT_HEADERS=['id', 'mtype','status_code','result','match_score','categories','chains','fsq_id','geocodes_main_latitude','geocodes_main_longitude','geocodes_roof_latitude','geocodes_roof_longitude','link','location_address','location_country','location_formatted_address','location_locality','location_neighborhood_0','location_postcode','location_region','name','related_places']
...
def normalize_json(data: dict) -> dict:
    ...
    if data is None:
        ...
        r = flatten(data)
        #logger.debug(f"Returning: {jsonify(r)}")
        #r = json_normalize(data)
    ...
    #                 new_data[key + "_" + k] = v
    #             else:
    #                 new_data[key + "_" + k]=normalize_json(v)
    #         # for k, v in value.items():
    #         #     new_data[key + "_" + k] = v
    # return new_data
...
def process_results(r):
    ...
    if status_code != 200:
        ...
        #logger.debug(f"\n\n***Normalized Results: {jsonify(rn)}")

        rn['result']='SUCCESS'

        RESULT['SUCCESS']=RESULT['SUCCESS']+1

    #logger.debug(f'NORMALIZED Result is: {jsonify(rn)}')       
    #print(f'NORMALIZED Result is: {jsonify(rn)}')       
...
def match(params,mtype="ALL"):
    ...
    #logger.debug(f"URL for id {id}: {request_url}") 
    ...
    if status_code == 200:
        ...
        return 204,{'id':id,'mtype':mtype,'status_code':status_code},mtype
    else:
        logger.info(f"[Match Type: {mtype}] Error: {id}")
        logger.info(f"Match Type: {mtype} Request Url:{request_url} Response Code: {status_code} Response Text:{response.text}")
...
if __name__ == "__main__":
    ...
    futures=[]
    with open(MY_FILE_INPUT, "rt",encoding='utf-8') as csv_in: #,encoding='utf-8-sig' needed sometimes with BOM characters etc.
        with open(MY_FILE_OUTPUT, "wt",encoding='utf-8') as csv_out:
            ...
            with ThreadPoolExecutor(N_THREADS) as executor:
                for row in csv.DictReader(csv_in,delimiter=separator_mapping[args.separator],escapechar='\\'): ###For PSV , dialect='piper'):
                    rcnt=rcnt+1
                    id=row[ID_COL]
                    logger.debug(f'===================START Processing > row: {rcnt} id:{id} ===================\n')
                    if rcnt < SKIP_TO:
                    ...
                    # logger.debug(f'#1 Params are: {jsonify(params)}')
                    ...
                    #Hack this if you want to compare results across match types and see which one does better
                    mtypes=["ALL"]
                    ...
                    logger.debug(f'===================FINISHED Processing > row: {rcnt} id:{id}===================\n')
    ...
    f=RESULT['FAILURE']
    t=s+f
    logger.info(f"Results - Success: {s} ({(s/t):0.2%}) / Failure: {f} ({(f/t):0.2%})")
